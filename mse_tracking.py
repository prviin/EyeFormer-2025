...
def train(
    ...
    metric_logger.add_meter(
        "loss_rl", utils.SmoothedValue(window_size=5, fmt="{value:.4f}")
    )
    metric_logger.add_meter(
        "rewards", utils.SmoothedValue(window_size=5, fmt="{value:.4f}")
    )
    ...
    for i, (image, saliency_image, width, height, raw_coord, raw_time) in enumerate(
...
def main(args, config):
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        if utils.is_main_process():
            ...
            # TODO: these are for evaluation only
            eval_stat_cmd = [
                "python",
            ...
            result = run(
                eval_stat_cmd, stdout=PIPE, stderr=PIPE, universal_newlines=True
            )
